...
export function verifiedCoverageTimestamp(payload, refreshedAt) {
  const observed = payload?.data?.lastCanonicalRefresh;
  if (
    typeof observed !== "string" ||
    !Number.isFinite(Date.parse(observed)) ||
    !Number.isFinite(Date.parse(refreshedAt)) ||
    Date.parse(observed) < Date.parse(refreshedAt)
  ) return null;
  return observed;
}
...
export function validatePreflightContract(payload) {
  ...
  if (
    ...
    mutation.phase_query !== "phase" ||
    !Array.isArray(mutation.phases) ||
    !mutation.phases.includes("discovery") ||
    !mutation.phases.includes("canonical")
  ) {
    throw new Error(
...
export async function postRefresh(url, headers, runKey, options = {}) {
  let postCount = 0;
  const sleep = options.sleep || defaultDelay;
  const conflictPollAttempts = Math.max(0, options.conflictPollAttempts ?? 60);
  const conflictPollDelayMs = Math.max(0, options.conflictPollDelayMs ?? 15_000);
  const conflictPollMaxDelayMs = Math.max(
    conflictPollDelayMs,
    options.conflictPollMaxDelayMs ?? 30_000
  );
  const request = () => fetchRetryable(url, {
      method: "POST",
  ...
  while (true) {
    ...
    const duplicateInProgress = response.status === 409 &&
      body?.error === "This refresh run is already in progress." &&
      body?.run_key === runKey;
    if (!duplicateInProgress) {
      throw new Error(`Refresh returned HTTP ${response.status}: ${JSON.stringify(body).slice(0, 500)}`);
    }
    if (conflictPolls >= conflictPollAttempts) {
      throw new Error(
        `Refresh remained in progress after ${conflictPollAttempts} conflict polls; ` +
        "refusing to generate a new idempotency key."
      );
    }
    conflictPolls += 1;
    const requestedDelay = retryAfterMilliseconds(response.headers.get("retry-after")) ??
      conflictPollDelayMs;
    await sleep(Math.min(requestedDelay, conflictPollMaxDelayMs));
  }
}
